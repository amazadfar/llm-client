...
from telic.hooks import BenchmarkRecorder, HookManager
from telic.memory import MemoryWrite, ShortTermMemoryStore
from telic.providers.types import (
    CompletionResult,
    EmbeddingResult,
    Message,
    StreamEvent,
...
from telic.structured import StructuredOutputConfig
from telic.structured_benchmarks import StructuredBenchmarkCase
from telic.tools import Tool, ToolExecutionEngine, ToolRegistry
from tests.telic.fakes import ScriptedProvider, ok_result


ARTIFACT_DIR = ROOT / "artifacts" / "benchmarks"
REPORT_PATH = ARTIFACT_DIR / "telic_rc_deterministic.json"
COMPARISON_PATH = ARTIFACT_DIR / "telic_rc_deterministic_comparison.json"
...
async def _run(*, write_baseline: bool = False) -> int:
    ...
    print("TELIC RC BENCHMARKS COMPLETED")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run deterministic telic release-candidate benchmarks.")
    parser.add_argument(
        "--write-baseline",
        action="store_true",
