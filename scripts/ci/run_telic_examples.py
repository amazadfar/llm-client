...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the live telic cookbook examples.",
    )
    parser.add_argument(
        "--subset",
...
def run_example(script_name: str, timeout_seconds: int) -> None:
    ...
    if env.get("TELIC_EXAMPLE_ALLOW_PERSISTENT_RESOURCES") != "1":
        env["TELIC_EXAMPLE_KEEP_UPLOADED_FILE"] = "0"
        env["TELIC_EXAMPLE_KEEP_VECTOR_STORE"] = "0"
        env["TELIC_EXAMPLE_KEEP_ANTHROPIC_FILE"] = "0"
    command = [sys.executable, str(script_path)]
    effective_timeout = max(timeout_seconds, TIMEOUT_OVERRIDES.get(script_name, timeout_seconds))
    print(f"[telic examples] running {script_name} timeout={effective_timeout}s", flush=True)
    try:
...
def main() -> int:
    ...
    print(f"[telic examples] completed {len(example_names)} example scripts for subset={subset}")
    return 0
