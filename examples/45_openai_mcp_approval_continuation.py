...
async def main() -> None:
    ...
    if handle.name != "openai":
        fail_or_skip("Set TELIC_EXAMPLE_PROVIDER=openai to run the OpenAI MCP approval continuation example.")

    previous_response_id = example_env("TELIC_EXAMPLE_MCP_PREVIOUS_RESPONSE_ID")
    approval_request_id = example_env("TELIC_EXAMPLE_MCP_APPROVAL_REQUEST_ID")
    approve = (example_env("TELIC_EXAMPLE_MCP_APPROVE", "1") or "1").strip().lower() not in {"0", "false", "no", "deny"}
    if not previous_response_id or not approval_request_id:
        fail_or_skip(
            "Set TELIC_EXAMPLE_MCP_PREVIOUS_RESPONSE_ID and "
            "TELIC_EXAMPLE_MCP_APPROVAL_REQUEST_ID to continue an MCP approval loop."
        )
