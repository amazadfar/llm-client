    ...
    try:
        ...
        try:
            ...
            await connection.create_text_message(
                "Acknowledge this realtime text turn in one short sentence.",
            ...
            for _ in range(2):
                try:
                    event = await connection.recv_until_type(
                        (
                            "session.updated",
                    ...
                    break
                observed_events.append(event.to_dict())
                if event.event_type == "response.done":
                    break

            print_heading("OpenAI Realtime Conversation Lifecycle")
            print_json(
                {
