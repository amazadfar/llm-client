...
from .lifecycle import LifecycleEvent, RequestReport, SessionReport
from .redaction import RedactionPolicy, sanitize_log_data
...
class StructuredLogger:
    ...
    def __init__(
        ...
        if redaction_policy is not None:
            ...
            self.redaction_policy = RedactionPolicy(sensitive_keys=())
    ...
    def _log(
        ...
        if data:
            record_data.update(sanitize_log_data(data, policy=self.redaction_policy))
    ...
    def log_lifecycle_event(self, event: LifecycleEvent) -> None:
        """Log a canonical lifecycle event with safe payload previews."""
        self._log(
            logging.INFO,
            f"Lifecycle event: {event.type.value}",
            event_type="lifecycle",
            data={
                "lifecycle_type": event.type.value,
    ...
    def log_request_report(self, report: RequestReport) -> None:
        """Log a normalized request report."""
        level = logging.INFO if report.success else logging.WARNING
        self._log(
            level,
            f"Request report: {report.request_id}",
            event_type="request_report",
            data=report.to_dict(),
        )

    def log_session_report(self, report: SessionReport) -> None:
        """Log a normalized session report."""
        self._log(
            logging.INFO,
    ...
    def log_error(
        ...
        **kwargs,
        ...
        # Extract additional info from TelicError
...
def get_logger(name: str = "telic") -> StructuredLogger:
