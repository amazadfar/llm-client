...
from ..redaction import ToolOutputPolicy, sanitize_tool_output
...
class LoggingMiddleware(ToolMiddleware):
    ...
    def __init__(self, logger: logging.Logger | None = None):
        self.logger = logger or logging.getLogger("telic.tools")
...
class RetryMiddleware(ToolMiddleware):
    ...
    async def __call__(self, ctx: ToolExecutionContext, next: NextHandler):
        ...
        last_error = None
        max_retries = int(ctx.metadata.get("retry_attempts", self.max_retries) or 0)
        for attempt in range(max_retries + 1):
            result = await next(ctx)
            if result.success:
                ctx.metadata["retry_attempts_used"] = attempt
                return result
            last_error = result.error
            if attempt < max_retries:
                await asyncio.sleep(self.base_backoff * (2 ** attempt))
        ctx.metadata["retry_attempts_used"] = max_retries
        return ToolResult.error_result(f"Failed after {max_retries + 1} attempts: {last_error}")
...
class ConcurrencyLimitMiddleware(ToolMiddleware):
    ...
    async def __call__(self, ctx: ToolExecutionContext, next: NextHandler):
        name = ctx.tool.name
        limit = ctx.metadata.get("concurrency_limit")
        if limit is None:
            limit = self._per_tool.get(name)
        if name not in self._tool_sems and limit:
            self._tool_sems[name] = asyncio.Semaphore(int(limit))
...
class ToolOutputPolicyMiddleware(ToolMiddleware):
    """Apply a unified tool-output redaction and truncation policy."""

    def __init__(self, policy: ToolOutputPolicy | None = None):
    ...
    async def __call__(self, ctx: ToolExecutionContext, next: NextHandler):
        ...
        sanitized = sanitize_tool_output(result.content, policy=self.policy)
        if sanitized == result.content:
            return result

        metadata = dict(result.metadata)
        metadata["tool_output_policy_applied"] = True
        metadata["tool_output_truncated"] = sanitized != result.content and isinstance(sanitized, str) and sanitized.endswith(self.policy.truncation_suffix)
        return ToolResult(
            content=sanitized,
...
class MiddlewareChain:
    ...
    def production_defaults(cls) -> "MiddlewareChain":
        ...
        telic usage where agent-runtime is NOT being used.
        ...
        - Standalone telic: Use production_defaults() via Agent(use_middleware=True)
