...
Prefer the stable package namespaces such as `telic.providers`,
`telic.engine`, `telic.agent`, and `telic.tools`.
"""
...
def _warn_container_compat_usage(symbol: str) -> None:
    warnings.warn(
        f"`telic.container.{symbol}` is a compatibility/integration surface and not "
        "part of the preferred standalone-package API. Prefer the stable module "
        "namespaces (`telic.providers`, `telic.engine`, `telic.agent`, "
        "`telic.tools`) for new code.",
        DeprecationWarning,
        stacklevel=2,
    )
