"""
Validation utilities for Telic.
...
def validate_spec(spec: RequestSpec) -> None:
    ...
    if provider_name and provider_name not in {"auto", "any", "unknown"} and spec.model:
        try:
            from .provider_registry import get_default_provider_registry

            registry = get_default_provider_registry()
            resolved_provider = registry.resolve_name(provider_name)
            if not registry.supports_model(resolved_provider, spec.model):
                raise ValidationError(
                    f"Model {spec.model!r} is not compatible with provider {resolved_provider!r}"
                )
