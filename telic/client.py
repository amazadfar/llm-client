...
from .content import ContentRequestEnvelope, content_blocks_to_text, ensure_content_response_envelope
...
class OpenAIClient:
    ...
    async def get_response(
        ...
        **kwargs: Any,
        ...
        # 4. Create canonical request envelope
        ...
        # STREAMING PATH
        if stream:
            return await self._handle_streaming(request, stream_mode, channel, self.engine)

        # COMPLETION PATH
        async def _do_complete():
            return await self.engine.complete_content(
                request,
        ...
        output_text = content_blocks_to_text(result_obj.message.blocks)
        if output_text:
            response_body["completion"] = output_text
    ...
    async def _handle_streaming(
        ...
        if mode == "sse":
            ...
            async def sse_gen():
                output = ""
                usage = {}
                yield format_sse_event("meta", json.dumps({"model": request.model, "stream_mode": "sse"}))

                async for event in engine.stream_content(request):
                    if event.type == StreamEventType.TOKEN:
                        ...
                        try:
                            envelope = ensure_content_response_envelope(event.data)
                        except TypeError:
                            envelope = None
                        if envelope is not None:
                            output = output or content_blocks_to_text(envelope.message.blocks)
