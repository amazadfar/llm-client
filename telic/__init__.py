"""
Telic - A production-ready library for LLM interactions.
...
For backward compatibility, the original `OpenAIClient` is still available:
    ```python
    from telic.compat import OpenAIClient

    client = OpenAIClient(model="gpt-5")
    result = await client.get_response(messages=[...])
    ```

Environment loading is explicit via `telic.config.load_env`.
...
try:  # Single source of truth is the installed distribution metadata (pyproject).
    ...
    __version__ = _dist_version("telic")
...
def __getattr__(name: str):
    ...
    warnings.warn(
        f"`telic.{name}` is compatibility-only. Import it from `telic.compat` instead.",
        DeprecationWarning,
        stacklevel=2,
    )
