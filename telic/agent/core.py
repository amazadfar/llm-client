...
from ..content import content_stream_event_to_completion_event
...
from .definition import AgentDefinition, AgentRuntimeState
...
class Agent:
    ...
    Example:
        ```python
        from telic import Agent, OpenAIProvider, tool
    ...
    def __init__(
        ...
        *,
        ...
        if provider is None and engine is None:
            raise ValueError("Agent requires a provider or an engine.")
        self.provider = provider if provider is not None else engine.provider
        if not use_engine:
            warnings.warn(
                "The 'use_engine=False' flag is deprecated and ignored. "
                "Agent now uses the ExecutionEngine by default.",
                DeprecationWarning,
                stacklevel=2,
            )
        self.engine: ExecutionEngine | None = engine if engine is not None else ExecutionEngine(provider=self.provider)
        ...
        base_definition = definition or AgentDefinition.from_agent_config(
        ...
        resolved_system_message = (
            ...
            else base_definition.system_message
            if base_definition.system_message is not None
            else conversation.system_message
        ...
        if config is not None:
            ...
            self.config = base_definition.to_agent_config()
        if max_turns != 10:
        ...
        resolved_tool_execution_mode = (
            base_definition.execution_policy.tool_execution_mode
            if definition is not None
            else None
        )

        self.definition = AgentDefinition.from_agent_config(
            self.config,
            name=base_definition.name,
            system_message=resolved_system_message,
            prompt_templates=base_definition.prompt_templates,
            metadata=dict(base_definition.metadata),
        )
        if (
            resolved_tool_execution_mode is not None
            and self.definition.execution_policy.tool_execution_mode is not resolved_tool_execution_mode
        ):
            self.definition = replace(
                self.definition,
                execution_policy=replace(
                    self.definition.execution_policy,
                    tool_execution_mode=resolved_tool_execution_mode,
                ),
            )
        ...
        if conversation:
            ...
            if resolved_system_message:
                active_conversation.system_message = resolved_system_message
        else:
            active_conversation = Conversation(
                system_message=resolved_system_message,
        ...
        self._runtime = AgentRuntimeState(conversation=active_conversation)
    ...
    def conversation(self) -> Conversation:
        return self._runtime.conversation

    @conversation.setter
    def conversation(self, value: Conversation) -> None:
        self._runtime.conversation = value
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        run_context = RequestContext.ensure(context)
        prior_request_context = self._runtime.request_context
        self._runtime.request_context = run_context
        base_idempotency_key = self._resolve_agent_idempotency_base_key(run_context, kwargs)
        ...
        try:
            ...
            for turn_num in range(max_turns):
                ...
                completion = await self._complete_with_engine(
                    engine_messages,
                    context=run_context,
                    idempotency_key=self._turn_idempotency_key(base_idempotency_key, turn_num, "completion"),
                    **dict(kwargs),
                )
            ...
            self._runtime.request_context = prior_request_context
        ...
        **kwargs: Any,
        ...
        try:
            ...
            for turn_num in range(max_turns):
                ...
                engine_messages = self.conversation.get_messages(model=self.model if self.config.max_tokens else None)
                request = build_content_request_envelope(
                    engine=self.engine,
                ...
                stream_iter = self.engine.stream_content(
                    request,
                    context=run_context,
                    idempotency_key=self._turn_idempotency_key(base_idempotency_key, turn_num, "stream"),
                )

                async def _normalized_stream() -> AsyncIterator[StreamEvent]:
                    async for event in stream_iter:
                        yield content_stream_event_to_completion_event(event)

                async for event in buffer.wrap(_normalized_stream()):
    ...
    async def _execute_tools(
        ...
        ctx = request_context or self._runtime.request_context
        ...
        results = await execute_tools(
            ...
            self.config,
            request_context=ctx,
            execution_mode=self.definition.execution_policy.tool_execution_mode,
        )
        # Apply output limits
        return [
        ...
        **kwargs: Any,
        ...
        return (
            await engine.complete_content(
                ...
                context=context or self._runtime.request_context,
    ...
    @staticmethod
    def _resolve_agent_idempotency_base_key(
    ...
    @staticmethod
    def _turn_idempotency_key(base_key: str | None, turn_num: int, operation: str) -> str | None:
        if not base_key:
            return None
        return f"{base_key}:turn-{turn_num}:{operation}"
