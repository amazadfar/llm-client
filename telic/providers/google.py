    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not GOOGLE_AVAILABLE:
            raise ImportError("google-genai is not installed. Install with `pip install telic[google]`")
    ...
    def _convert_messages(messages: list[Message]) -> tuple[str | None, list[Any]]:
        ...
        for msg in messages:
            ...
            if msg.role != Role.TOOL:
                parts.extend(GoogleProvider._content_blocks_to_google_parts(blocks))
            ...
            if msg.tool_call_id:
                ...
                tool_name = (tool_result.name if tool_result else None) or msg.name or "unknown_tool"

                # Try to parse content as JSON if it looks like it, otherwise string
                raw_content = tool_result.content if tool_result else content_blocks_to_text(blocks)
                try:
                    response_content = json.loads(raw_content) if raw_content else {}
                except Exception:
                    response_content = {"result": raw_content}
        ...
        async def _make_request() -> CompletionResult:
            ...
            async with self.limiter.limit(tokens=input_tokens, requests=1) as limit_ctx:
                try:
                    ...
                    return failure_to_completion_result(self._failure(e, operation="complete"), model=self.model_name)
        ...
        async def _make_request() -> EmbeddingResult:
            ...
            async with self.limiter.limit(tokens=input_tokens, requests=1):
                try:
                    ...
                    return failure_to_embedding_result(self._failure(e, operation="embed"), model=embed_model)
                except Exception as e:
                    return failure_to_embedding_result(self._failure(e, operation="embed"), model=embed_model)
