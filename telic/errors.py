...
class ErrorCode(str, Enum):
    """Standardized error codes for the Telic."""
...
class TelicError(Exception):
    """
    Base exception for all Telic errors.
...
class ProviderError(TelicError):
...
class ValidationError(TelicError):
...
def normalize_exception(
    ...
    *,
    ...
    if isinstance(error, TelicError):
        return error.to_failure()
...
def _remediation_hint_for_error_code(code: ErrorCode) -> str | None:
    mapping = {
        ...
        ErrorCode.INVALID_CONFIG: "Fix the telic configuration before retrying.",
    }
