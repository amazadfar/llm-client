...
Compatibility Note
------------------
This module is retained for application-integration compatibility. It is not
part of the preferred standalone-package API contract for new projects.
Prefer the stable package namespaces such as `llm_client.providers`,
`llm_client.engine`, `llm_client.agent`, and `llm_client.tools`.
"""
...
def _warn_container_compat_usage(symbol: str) -> None:
    warnings.warn(
        f"`llm_client.container.{symbol}` is a compatibility/integration surface and not "
        "part of the preferred standalone-package API. Prefer the stable module "
        "namespaces (`llm_client.providers`, `llm_client.engine`, `llm_client.agent`, "
        "`llm_client.tools`) for new code.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def create_openai_provider(
    ...
    **kwargs: Any,
    ...
    """
    _warn_container_compat_usage("create_openai_provider")
    from .provider_registry import get_default_provider_registry

    return get_default_provider_registry().create("openai", model=model, api_key=api_key, **kwargs)
...
def create_anthropic_provider(
    ...
    **kwargs: Any,
    ...
    """
    _warn_container_compat_usage("create_anthropic_provider")
    from .provider_registry import get_default_provider_registry

    return get_default_provider_registry().create("anthropic", model=model, api_key=api_key, **kwargs)
...
def create_google_provider(
    ...
    **kwargs: Any,
    ...
    """
    Factory function to create a Google provider.
    ...
    """
    _warn_container_compat_usage("create_google_provider")
    from .provider_registry import get_default_provider_registry

    return get_default_provider_registry().create("google", model=model, api_key=api_key, **kwargs)
...
def create_provider(
    ...
    **kwargs: Any,
    ...
    """
    _warn_container_compat_usage("create_provider")
    from .provider_registry import get_default_provider_registry

    return get_default_provider_registry().create(provider_name, model=model, **kwargs)
...
def create_cache(
    ...
    **kwargs: Any,
    ...
    Returns:
        CacheCore instance
    """
    _warn_container_compat_usage("create_cache")
...
def create_agent(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        provider_name: "openai", "anthropic", or "google" (if no provider given)
        model: Model name
        tools: List of tools
    ...
    """
    _warn_container_compat_usage("create_agent")
...
class Container:
    ...
    _google_provider: Any | None = field(default=None, repr=False)
    ...
    def from_config(cls, settings: Any | None = None) -> Container:
        ...
        """
        _warn_container_compat_usage("Container.from_config")
    ...
    def default(cls) -> Container:
        ...
        _warn_container_compat_usage("Container.default")
    ...
    def openai_provider(self, **kwargs):
        ...
        if self._openai_provider is None or kwargs:
            ...
            self._openai_provider = create_openai_provider(
                ...
                model=kwargs.get("model") or (config.default_model if config else "gpt-5"),
    ...
    def google_provider(self, **kwargs):
        """Get or create Google provider."""
        if self._google_provider is None or kwargs:
            from .config import Settings

            settings = self.registry.try_resolve(Settings)
            config = settings.google if settings else None

            self._google_provider = create_google_provider(
                api_key=kwargs.get("api_key") or (config.api_key if config else None),
                model=kwargs.get("model") or (config.default_model if config else "gemini-2.0-flash"),
                **{k: v for k, v in kwargs.items() if k not in ("api_key", "model")},
            )
    ...
    def provider(self, name: str = "openai", **kwargs):
        ...
        if name == "openai":
            ...
            return self.google_provider(**kwargs)
...
def get_container() -> Container:
    ...
    _warn_container_compat_usage("get_container")
...
def set_container(container: Container) -> None:
    ...
    _warn_container_compat_usage("set_container")
    global _default_container
