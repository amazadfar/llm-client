...
def _default_model_for(provider: str, *, category: str = "completions", fallback: str) -> str:
    try:
        from ..model_catalog import get_provider_default_model

        return get_provider_default_model(provider, category=category) or fallback
    except Exception:
        return fallback
...
class OpenAIConfig(ProviderConfig):
    ...
    default_model: str = field(default_factory=lambda: _default_model_for("openai", fallback="gpt-5"))
...
class GoogleConfig(ProviderConfig):
    ...
    default_model: str = field(default_factory=lambda: _default_model_for("google", fallback="gemini-2.0-flash"))
