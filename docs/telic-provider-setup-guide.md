# telic Provider Setup Guide

This guide covers the provider side of `telic`: direct provider usage,
...
```python
from telic.providers import OpenAIProvider, AnthropicProvider, GoogleProvider
from telic.providers.types import BackgroundResponseResult, Message, StreamEventType
...
Responses tool descriptors from `telic.tools`:
...
`ResponsesApplyPatchCallOutput` are available from `telic.tools` so you do
...
```bash
export TELIC_EXAMPLE_PROVIDER=openai
export TELIC_EXAMPLE_MODEL=gpt-5-mini
export TELIC_EXAMPLE_SECONDARY_PROVIDER=openai
export TELIC_EXAMPLE_SECONDARY_MODEL=gpt-5-nano
export TELIC_EXAMPLE_EMBEDDINGS_PROVIDER=openai
export TELIC_EXAMPLE_EMBEDDINGS_MODEL=text-embedding-3-small
```
...
export TELIC_EXAMPLE_PROVIDER=anthropic
export TELIC_EXAMPLE_MODEL=claude-sonnet-4-6
```
...
export TELIC_EXAMPLE_PROVIDER=google
export TELIC_EXAMPLE_MODEL=gemini-2.0-flash
```
