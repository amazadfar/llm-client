# telic 0.2.0 Release Notes

Last updated: 2026-04-01

`telic` `0.2.0` extends the pre-`1.0` package line with a broader
...
- OpenAI provider product/workflow APIs:
  ...
  - `start_deep_research(...)`
  - `run_deep_research(...)`
- Stable OpenAI Responses tool descriptors in `telic.tools`:
...
```bash
./.venv/bin/pytest -q \
  tests/telic/test_openai_provider_contract_smoke.py \
  tests/telic/test_provider_overlap_contracts.py \
  tests/telic/test_openai_content_translation.py \
  tests/telic/test_provider_response_parsing.py \
  tests/telic/test_provider_request_translation.py \
  tests/telic/test_file_block_transport.py \
  tests/telic/test_content_model.py \
  tests/telic/test_public_api_namespaces.py \
  tests/telic/test_guides_inventory.py \
  tests/telic/test_request_builders.py \
  tests/telic/test_model_catalog.py \
...
```bash
./.venv/bin/pytest -q \
  tests/telic/test_engine_reliability.py \
  tests/telic/test_openai_provider_contract_smoke.py \
  tests/telic/test_provider_overlap_contracts.py \
  tests/telic/test_openai_content_translation.py \
  tests/telic/test_provider_response_parsing.py \
...
```bash
./.venv/bin/pytest -q \
  tests/telic/test_openai_extended_api_surfaces.py \
  tests/telic/test_engine_reliability.py \
  tests/telic/test_public_api_namespaces.py \
  tests/telic/test_model_catalog.py
```
...
skip under `TELIC_EXAMPLE_ALLOW_SKIP=1`. The heavier
...
TELIC_EXAMPLE_ALLOW_SKIP=1 \
./.venv/bin/python scripts/ci/run_telic_examples.py --subset all --timeout-seconds 120
```
