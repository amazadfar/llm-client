# telic Build and Recipes Guide

This is the practical "how do I build with this package?" guide.

Use it together with:

- [telic-package-api-guide.md](telic-package-api-guide.md)
- [telic-usage-and-capabilities-guide.md](telic-usage-and-capabilities-guide.md)
...
```bash
pip install telic[anthropic,google]
pip install telic[postgres]
pip install telic[mysql]
pip install telic[redis]
pip install telic[qdrant]
pip install telic[telemetry]
pip install telic[server]
pip install telic[all]
```
...
from telic.providers import OpenAIProvider
from telic.types import Message, RequestSpec
...
async def main() -> None:
    ...
    spec = RequestSpec(
        provider="openai",
        model="gpt-5-mini",
        messages=[Message.user("Summarize this in one sentence: telic is now stable.")],
    )
...
from telic.providers import OpenAIProvider
from telic.structured import extract_structured
...
```python
from telic.content import ContentMessage, FileBlock, TextBlock
from telic.providers.types import Role
...
from telic.cache import CachePolicy, CacheSettings, build_cache_core
from telic.engine import ExecutionEngine
from telic.providers import OpenAIProvider
from telic.types import Message, RequestSpec
...
```python
from telic.cache import CacheInvalidationMode, CachePolicy, request_cache_key
...
from telic.budgets import Budget, Ledger
from telic.context import ExecutionContext
...
```python
from telic.observability import EngineDiagnosticsRecorder, LifecycleRecorder
...
```python
from telic.adapters import PostgresSQLAdaptor, SQLMutationRequest, SQLQueryRequest
...
```python
from telic.adapters import PostgresSQLAdaptor
from telic.adapters.tools import build_sql_query_tool
...
```python
from telic.adapters import RedisKVAdaptor, RedisGetRequest, RedisSetRequest
...
```python
from telic.adapters import QdrantVectorAdaptor, VectorPoint, VectorSearchRequest, VectorUpsertRequest
