# Migrate from Direct SDK Usage to telic

This guide is for teams currently calling vendor SDKs directly and wanting to
move onto `telic` without losing control of low-level behavior.
...
`telic` centralizes those concerns.
...
3. migrate structured output flows onto `telic.structured`
4. migrate tool flows onto `telic.tools` / `telic.agent`
