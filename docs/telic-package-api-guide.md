# telic Package API Guide

This guide explains the standalone package contract for `telic` as a
...
Treat `telic.compat` as compatibility-only and `telic.advanced` as
...
Use `telic.types` and `telic.content` for canonical request data:
...
Use `telic.engine` when you want:
...
Use `telic.budgets` when you need:
...
Use `telic.engine` instead of direct provider calls when you want those
...
Use `telic.adapters` when you need normalized access to supporting
...
- `telic.context` for execution envelope and budget/policy metadata
- `telic.context_assembly` for generic multi-source assembly contracts
- `telic.memory` for generic memory and summary abstractions

### Observability

Use `telic.observability` for:
...
- `telic.providers`
- or `telic.engine` if you want retry/cache/failover
...
- `telic.structured`
- optionally through `telic.engine`
...
- `telic.agent` if the adaptor will be exposed to a model through a tool
...
Top-level `telic` imports are allowed as convenience imports, but they are
...
```python
from telic import ExecutionEngine, OpenAIProvider, tool
```

Compatibility-only imports should remain under `telic.compat`.

## What Is Outside The Package Contract

`telic` does not define the contract for:
...
Those layers may consume `telic`, but they are not the package API.
