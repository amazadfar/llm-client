# telic 0.3.1 Release Notes

Last updated: 2026-05-25

`telic` `0.3.1` is a patch release for provider catalog correctness,
...
```bash
./.venv/bin/pytest -q tests/telic
```

Result: `354 passed, 3 skipped, 6 warnings`

```bash
.venv/bin/python -m compileall telic
```
