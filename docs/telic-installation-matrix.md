# telic Installation Matrix

This document defines the supported installation shapes for the standalone
`telic` package.

## Supported Python Versions

- Python `3.10`
- Python `3.11`
- Python `3.12`

These versions are validated in CI for installation and package smoke checks.

## Provider SDK Versions

| Provider | Supported range | Release-candidate version |
|---|---|---|
| OpenAI | `openai>=2.36,<3` | `2.36.0` |
| Anthropic | `anthropic>=0.104,<1` | `0.104.1` |

The lower bounds match the first SDK versions validated for all `0.4.0`
provider resources and request parameters.

## Base Install

Use the base package when you want the core runtime, OpenAI provider path,
engine, agent loop, tools, content model, cache APIs, context/memory APIs,
observability, and benchmarks.

The base install intentionally excludes optional PostgreSQL and Redis runtime
dependencies. Install those extras only if you need the Postgres-backed
persistence paths or the `pg_redis` hot-cache layer.

```bash
pip install telic
```

Editable local install:

```bash
pip install -e .
```

## Optional Extras

### Anthropic

```bash
pip install "telic[anthropic]"
```

### Google

```bash
pip install "telic[google]"
```

### PostgreSQL Adaptor

```bash
pip install "telic[postgres]"
```

### MySQL Adaptor

```bash
pip install "telic[mysql]"
```

### Redis Adaptor

```bash
pip install "telic[redis]"
```

Use this alongside `telic[postgres]` if you want the full
`pg_redis` hot-cache path. Without the Redis extra, the Postgres-backed cache
can still operate in durable-storage mode without Redis acceleration.

### Qdrant Adaptor

```bash
pip install "telic[qdrant]"
```

### Adaptor Bundle

```bash
pip install "telic[adapters]"
```

### Telemetry

```bash
pip install "telic[telemetry]"
```

### Performance

```bash
pip install "telic[performance]"
```

### Server / FastAPI

```bash
pip install "telic[server]"
```

### Development

```bash
pip install -e ".[dev]"
```

### Full Feature Set

```bash
pip install -e ".[anthropic,google,postgres,mysql,redis,qdrant,telemetry,performance,server,dev]"
```

Or:

```bash
pip install -e ".[all]"
```

## Validation Matrix

The release/readiness process validates these shapes locally:

- base install on Python `3.10`, `3.11`, and `3.12`
- full-feature install on Python `3.11`
- live cookbook entrypoint validation on Python `3.11` with credential-less skip allowance
- wheel and sdist build verification on Python `3.11`

## Notes

- The base install intentionally includes the core cache/runtime dependencies
  required by the current stable `telic.cache` namespace.
- Provider-specific SDKs for Anthropic and Google remain optional extras.
- Service adaptor integrations for PostgreSQL, MySQL, Redis, and Qdrant are
  installable by extras.
- Observability integrations such as OpenTelemetry and Prometheus remain
  optional extras.
- A reusable local service stack is available at
  [docker-compose.telic-services.yml](../docker-compose.telic-services.yml)
  for PostgreSQL, Redis, Qdrant, and MySQL-backed package development.
