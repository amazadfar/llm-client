...
from telic.content import ContentRequestEnvelope, ContentResponseEnvelope
from telic.providers.types import CompletionResult, StreamEvent, StreamEventType, Usage
from telic.tools.runtime import (
    RuntimeToolError,
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_streaming_extracts_assistant_text_token_deltas() -> None:
    callbacks: list[str] = []
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_streaming_redacts_internal_ids_from_assistant_text() -> None:
    callbacks: list[str] = []
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_engine_completion_uses_content_envelopes() -> None:
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_engine_streaming_uses_content_envelopes() -> None:
    engine = _FakeEnvelopeEngine()
    callbacks: list[str] = []
...
def test_telic_tool_runtime_converts_tools_to_provider_defs() -> None:
...
def test_telic_tool_runtime_argument_validation_accepts_contract_registries() -> None:
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_rejects_malformed_tool_calls() -> None:
    completion, error, messages_out = await complete_with_tools(
        engine=None,
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_enforces_depth_limits() -> None:
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_enforces_allowlist_for_declared_tools() -> None:
    provider_tool = _FakeTool().to_openai_format()
...
@pytest.mark.asyncio
async def test_telic_tool_runtime_timeout_normalizes_failure() -> None:
