...
from telic import adapters, budgets, compat, config, content, engine, models, observability, providers, tools, types, validation
from telic.compat import OpenAIClient, ResponseTimeoutError
from telic.adapters import PostgresSQLAdaptor, SQLQueryRequest
from telic.content import Message, Role, ToolCall, normalize_messages
...
def test_top_level_compat_aliases_warn_and_resolve() -> None:
    with warnings.catch_warnings(record=True) as caught:
        ...
        legacy_client = telic.OpenAIClient
...
def test_top_level_all_is_now_a_smaller_stable_contract() -> None:
    assert "Agent" in telic.__all__
    assert "OpenAIProvider" in telic.__all__
    assert "OpenAIClient" in telic.__all__
    assert "TelemetryCounter" not in telic.__all__
    assert "fingerprint" not in telic.__all__
    assert not hasattr(telic, "TelemetryCounter")
    assert not hasattr(telic, "fingerprint")
    assert not hasattr(telic, "BatchManager")
