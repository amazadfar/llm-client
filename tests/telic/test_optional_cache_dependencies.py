...
def test_cache_namespace_import_does_not_eagerly_load_pg_redis_backend() -> None:
    sys.modules.pop("telic.cache", None)
    sys.modules.pop("telic.cache.postgres_redis", None)

    cache_module = importlib.import_module("telic.cache")

    assert cache_module.CacheCore is not None
    assert "telic.cache.postgres_redis" not in sys.modules


def test_persistence_module_imports_without_asyncpg(monkeypatch: pytest.MonkeyPatch) -> None:
    sys.modules.pop("telic.persistence", None)
    ...
    persistence_module = importlib.import_module("telic.persistence")
...
async def test_pg_redis_backend_requires_asyncpg_only_when_used(monkeypatch: pytest.MonkeyPatch) -> None:
    backend_module = importlib.import_module("telic.cache.postgres_redis")
    ...
    with pytest.raises(ModuleNotFoundError, match=r"telic\[postgres\]"):
